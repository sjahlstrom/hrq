...
const FinalAnalysis: React.FC<FinalAnalysisProps> = ({
    ...
    const allScales = useMemo(() => [
        scales, scales2, scales3, scales4, scales5, scales6,
        scales7, scales8, scales9, scales10, scales11, scales12
    ], [])
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-2 p-4">
                {allScales.map((scales, index) => (
                    <div key={index} className="w-full px-1">
                        <MemoizedBChart scales={scales} />
                    </div>
                ))}
            </div>
            <Button>Generate PDF</Button>
