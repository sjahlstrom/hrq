...
//
//     // This has no effect, but keep it around in case I want to use it for redirect or other things
//     useEffect(() => {
...
//             }, 300) // Wait for fade out before changing question
//         }
//     }, [
//         isLastQuestion,
//         currentQuestion,
    ...
    <div className="relative w-full h-8 mt-2">
        {Object.entries(options).map(([key, value], index) => (
            <div
                key={key}
                className={`absolute transform -translate-y-1/2 text-xs sm:text-sm ${
                    index === 0 ? 'left-16' : index === 1 ? 'left-1/2 -translate-x-1/2' : 'right-10'
                }`}
                style={{
                    top: '50%',
                    left: index === 1 ? 'calc(50% + 10px)' : undefined,
...
export default function TheTest({
    ...
    const updateSliderValue = useCallback((value: number | number[]) => {
        ...
        setState(prev => ({
            ...prev,
            [currentQuestion.reverse ? 'reverseSliderValue' : 'sliderValue']: newValue,
            isSliderUsed: true,
        }))
    }, [currentQuestion.reverse])
            ...
            <div className="flex-grow flex flex-col justify-between items-center px-4 py-8 md:py-16">
                ...
                <div className="w-full max-w-3xl flex justify-center mb-8 md:mb-12">
                    <Button
                        ...
                        onClick={isLastQuestion ? handleShowAnalysis : advanceToNextQuestion}
                        disabled={!isLastQuestion && !state.isSliderUsed}
                    >
                        <span className={`relative z-10 ${!state.isSliderUsed && !isLastQuestion ? 'pointer-events-none' : ''}`}>
                            {isLastQuestion ? 'Show Analysis' : 'Next Question'}
                        </span>
                        <span className="absolute inset-0 overflow-hidden rounded-xl">
                            <span
                                className={`absolute left-0 w-full h-full origin-center -translate-x-full rounded-full ${
                                    isLastQuestion ? 'bg-hrqColors-skyBlue-800' : 'bg-air_force_blue-300'
                                } transition-transform duration-500 ${
