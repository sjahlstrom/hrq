...
const cannedScaleStatements: ScaleStatement[] =
    rawCannedScaleStatements as ScaleStatement[]
...
export function BChart({ scales }: BChartProps) {
    ...
    const getStatement = useMemo(
        () =>
            (scaleName: string, score: number): string => {
                const scaleStatements = cannedScaleStatements.filter(
                    (scale) => scale.name === scaleName
                )
                if (scaleStatements.length === 0)
                    return 'No statement available for this scale.'

                const statement = scaleStatements.find(
    ...
    const scoreCache = new Map<number, number>() // Ensuring key type is number if scale.number is numeric

    const fetchData = useCallback(
        async (scalesArray: Scale[]) => {
            try {
                // Create an array of promises for each scale
                const scoresPromises = scalesArray.map(async (scale) => {
                    // Check the cache for an existing score
                    if (scoreCache.has(scale.number)) {
                        return scoreCache.get(scale.number);
                    }

                    // Find positions for the scale
                    ...
                    // Log fetching data only in development
                    if (process.env.NODE_ENV === 'development') {
                        console.log('Fetching data...');
                    }

                    // Fetch and cache the result
                    const result = await sumTestResponsesAtPositions([pos1, pos2]);
                    scoreCache.set(scale.number, result);
                    return result;
                });
        ...
        [getStatement] // Ensure that any functions used inside the callback are included here
    );

    useEffect(() => {
        const loadData = async () => {
            try {
                const scalesArray = Array.isArray(scales)
                    ? scales
        ...
        <div>
            ...
            <div className="mt-8 space-y-4">
                {chartData.map((item, index) => (
                    <Card
                        key={index}
                        className="bg-hrqColors-skyBlue-600 border-hrqColors-sunsetOrange-500 rounded-2xl shadow-xl"
                    >
                        <CardHeader>
                            <CardTitle className="text-hrqColors-skyBlue-900">
                                {item.scale} Scale
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
